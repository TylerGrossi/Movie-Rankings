"""
Movie Score Predictor - V6 (Score + Star Percentage)
=====================================================
New features:
- Director_Avg_Score, Writer_Avg_Score, Actor_Avg_Score (lead)
- Genre1–3 averages from Genre Rankings; Creative_Core_Score (mean of those six pillars); Combined_Personal_Score (60% D/W/A, 40% split across genres)
- Supporting cast mean still computed for possible future use, not modeled by default feature list
- Learned keyword weights from YOUR data
- XGBoost regression for predicted score; separate classifier for Star Percentage (P(My_Score >= 8.0))

Requirements:
    pip install pandas scikit-learn openpyxl joblib xgboost
...
# Trim extremes: map predictions to the inner range of your ratings (0.1 = 10th–90th percentile).
# Keeps distribution shape but avoids overly high highs and low lows.
CALIBRATION_TRIM = 0.10

# Star probability: P(you rate >= this). Yes requires Predicted Score >= this too.
STAR_THRESHOLD = 8.0
# Star % rises linearly from score 8→10 (from your data), plus classifier nudge.
STAR_SCORE_ANCHOR_LO = 8.0
STAR_SCORE_ANCHOR_HI = 10.0
STAR_CLF_NUDGE = 0.16          # max swing from classifier vs your base rate
STAR_PROB_CAP = 0.94
STAR_PROB_FLOOR = 0.02
...
# ============================================================
# LOAD RANKS FROM EXCEL (Movies Ranks.xlsm)
# ============================================================

def load_ranks_from_excel(filepath=None):
    """Load Director, Actor, and Genre scores from Movies Ranks.xlsm.
    Returns dict with 'director', 'actor', 'genre1', 'genre2', 'genre3' (all genre slots use Genre Rankings).
    Uses 'Rating' for directors/genres; for actors uses a +Rating column (+Rating or '+Rating in the sheet) when present, else 'Rating'.
    Raises if file missing or required sheets invalid.
    """
    path = filepath or RANKS_FILE
    if not os.path.exists(path):
        raise FileNotFoundError(f"Movies Ranks file not found: {path}")
    xl = pd.ExcelFile(path)
    ranks = {}
    # Director Rankings
    if 'Director Rankings' not in xl.sheet_names:
        raise ValueError(f"Movies Ranks.xlsm must have 'Director Rankings' sheet. Found: {xl.sheet_names}")
    df = pd.read_excel(xl, sheet_name='Director Rankings', header=0)
    if 'Director' not in df.columns or 'Rating' not in df.columns:
        raise ValueError(f"Director Rankings sheet must have 'Director' and 'Rating' columns. Found: {list(df.columns)}")
    ranks['director'] = df.dropna(subset=['Director', 'Rating']).set_index('Director')['Rating'].to_dict()
    # Actor Rankings
    if 'Actor Rankings' not in xl.sheet_names:
        raise ValueError(f"Movies Ranks.xlsm must have 'Actor Rankings' sheet. Found: {xl.sheet_names}")
    df = pd.read_excel(xl, sheet_name='Actor Rankings', header=0)
    actor_col = 'Best Actors' if 'Best Actors' in df.columns else 'Actor'
    if actor_col not in df.columns or 'Rating' not in df.columns:
        raise ValueError(f"Actor Rankings sheet must have '{actor_col}' and 'Rating' columns. Found: {list(df.columns)}")
    plus_col = None
    for cand in ('+Rating', "'+Rating"):
        if cand in df.columns:
            plus_col = cand
            break
    if plus_col is None:
        for c in df.columns:
            if isinstance(c, str) and c.replace("'", '').strip().lower() == '+rating':
                plus_col = c
                break
    base_rating = pd.to_numeric(df['Rating'], errors='coerce')
    if plus_col is not None:
        plus_rating = pd.to_numeric(df[plus_col], errors='coerce')
        actor_values = plus_rating.where(plus_rating.notna(), base_rating)
    else:
        actor_values = base_rating
    actor_df = df[[actor_col]].copy()
    actor_df['_score'] = actor_values
    actor_df = actor_df.dropna(subset=[actor_col])
    actor_df = actor_df[actor_df['_score'].notna()]
    ranks['actor'] = actor_df.set_index(actor_col)['_score'].to_dict()
    # Genre Rankings
    if 'Genre Rankings' not in xl.sheet_names:
        raise ValueError(f"Movies Ranks.xlsm must have 'Genre Rankings' sheet. Found: {xl.sheet_names}")
    df = pd.read_excel(xl, sheet_name='Genre Rankings', header=0)
    genre_col = 'Genres' if 'Genres' in df.columns else 'Genre'
    if genre_col not in df.columns or 'Rating' not in df.columns:
        raise ValueError(f"Genre Rankings sheet must have '{genre_col}' and 'Rating' columns. Found: {list(df.columns)}")
    genre_scores = df.dropna(subset=[genre_col, 'Rating']).set_index(genre_col)['Rating'].to_dict()
    ranks['genre1'] = genre_scores
...
def detect_franchise(title):
    ...
    franchises = {
        ...
        'avengers': 'Avengers',  # Team-up/crossover films get their own franchise (scored above Marvel)
        'iron man': 'Marvel', 'captain america': 'Marvel',
...
def compute_personal_scores(df, excel_ranks):
    """Compute average scores for directors, actors, genres, etc.
    excel_ranks (from Movies Ranks.xlsm) is required for director/actor/genre scores.
    Other scores (writer, company, franchise, etc.) still computed from ratings.
    """
    scores = {}
    ...
    # Avengers: no rated titles yet, so set score above Marvel (team-up films "combine all of them")
    marvel_avg = scores['franchise'].get('Marvel', overall_mean)
    if 'Avengers' not in scores['franchise']:
        scores['franchise']['Avengers'] = min(marvel_avg + 0.5, 10.0)  # boost above other Marvel
    
    # === DIRECTOR SCORES (from Movies Ranks.xlsm) ===
    scores['director'] = excel_ranks['director']
    ...
    # === ACTOR SCORES (from Movies Ranks.xlsm) ===
    actor_scores = excel_ranks['actor']
    scores['actor'] = actor_scores
    
    # === GENRE SCORES (from Movies Ranks.xlsm — same lookup for Genre1–3 slots) ===
    scores['genre1'] = excel_ranks['genre1']
    scores['genre2'] = excel_ranks.get('genre2', excel_ranks['genre1'])
    scores['genre3'] = excel_ranks.get('genre3', excel_ranks['genre1'])
...
def compute_keyword_weights(df):
    ...
    Uses weighted formula similar to genres:
    <10 occurrences: (Score - 1.5) / Count
    10-19 occurrences: (Score + 1.5) / Count
    20-29 occurrences: (Score + 3) / Count
    ≥30 occurrences: (Score + 5) / Count
    """
    if 'Keywords' not in df.columns:
        return {}, {}
    
    # Collect all keywords and their associated scores
    keyword_data = {}  # {keyword: {'total': sum, 'count': n}}
        ...
        for kw in str(keywords).lower().split(', '):
            ...
            if kw:
                if kw not in keyword_data:
                    keyword_data[kw] = {'total': 0, 'count': 0}
                keyword_data[kw]['total'] += score
                keyword_data[kw]['count'] += 1
    
    # Calculate weighted score per keyword (min 3 occurrences)
    overall_mean = df['My_Score'].mean()
    ...
    for kw, data in keyword_data.items():
        ...
        if count >= 3:
            # Apply weighted formula (same as genres)
            if count < 10:
                weighted = (total_score - 1.5) / count
            elif count < 20:
                weighted = (total_score + 1.5) / count
            ...
            diff = weighted - overall_mean
            
            if diff > 0.3:  # You rate these keywords above average
                positive_keywords[kw] = {'weight': diff, 'count': count, 'weighted_score': weighted}
            elif diff < -0.3:  # You rate these keywords below average
                negative_keywords[kw] = {'weight': diff, 'count': count, 'weighted_score': weighted}
    
    # Sort and print top keywords
    pos_sorted = sorted(positive_keywords.items(), key=lambda x: x[1]['weight'], reverse=True)[:15]
    neg_sorted = sorted(negative_keywords.items(), key=lambda x: x[1]['weight'])[:15]
    
    print("\n  Keywords you LIKE (weighted score above average):")
    for kw, data in pos_sorted[:10]:
        print(f"    +{data['weight']:.2f}: {kw} ({data['count']} movies, weighted: {data['weighted_score']:.2f})")
    
    print("\n  Keywords you DISLIKE (weighted score below average):")
    for kw, data in neg_sorted[:10]:
        print(f"    {data['weight']:.2f}: {kw} ({data['count']} movies, weighted: {data['weighted_score']:.2f})")
    
    # Convert to simple weight dicts for use in scoring
    pos_weights = {kw: data['weight'] for kw, data in positive_keywords.items()}
    neg_weights = {kw: data['weight'] for kw, data in negative_keywords.items()}
...
def prepare_features(df, training_stats=None, personal_scores=None, 
    ...
    # Median imputation for numeric columns (IMDb / RT / Metascore stay NaN when missing)
    no_impute_cols = {'IMDb', 'RT', 'Metascore'}
    if is_training:
        training_stats = {
            col: df[col].median()
            for col in num_cols
            if col in df.columns and col not in no_impute_cols
        }
    ...
    if act_map is not None:
        ...
        df['Supporting_Cast_Avg_Score'] = overall_mean
    ...
    if 'Genre3' in df.columns and personal_scores and 'genre3' in personal_scores:
        df['Genre3_Avg_Score'] = df['Genre3'].map(personal_scores['genre3']).fillna(overall_mean)
    else:
        df['Genre3_Avg_Score'] = overall_mean
    ...
    # Single index of director / writer / lead / all three billed genres (equal weight).
    df['Creative_Core_Score'] = (
        df['Director_Avg_Score'] + df['Writer_Avg_Score'] + df['Actor_Avg_Score'] +
        df['Genre1_Avg_Score'] + df['Genre2_Avg_Score'] + df['Genre3_Avg_Score']
    ) / 6.0
    
    # ===== COMBINED PERSONAL SCORE — same pillars, emphasizes director/writer/lead equally, then genres =====
    g_share = 0.40 / 3.0
    df['Combined_Personal_Score'] = (
        df['Director_Avg_Score'] * 0.20 +
        df['Writer_Avg_Score'] * 0.20 +
        df['Actor_Avg_Score'] * 0.20 +
        df['Genre1_Avg_Score'] * g_share +
        df['Genre2_Avg_Score'] * g_share +
        df['Genre3_Avg_Score'] * g_share
    )
    
    # ===== CORE SCORE FEATURES =====
    imdb100 = df['IMDb'] * 10
    df['Critics_Avg'] = pd.concat([imdb100, df['RT'], df['Metascore']], axis=1).mean(axis=1, skipna=True)
    df['IMDb_RT_Diff'] = imdb100 - df['RT']
    df['IMDb_Meta_Diff'] = imdb100 - df['Metascore']
    ...
    # ===== QUALITY INDICATORS =====
    df['High_IMDb'] = np.where(df['IMDb'].notna(), (df['IMDb'] >= 7.5).astype(int), 0)
    df['High_RT'] = np.where(df['RT'].notna(), (df['RT'] >= 80).astype(int), 0)
    df['High_Meta'] = np.where(df['Metascore'].notna(), (df['Metascore'] >= 70).astype(int), 0)
...
def get_feature_columns():
    return [
        # === Creative core (prioritize in feature order — director/writer/lead + all genres) ===
        'Director_Avg_Score', 'Writer_Avg_Score', 'Actor_Avg_Score',
        'Genre1_Avg_Score', 'Genre2_Avg_Score', 'Genre3_Avg_Score',
        'Creative_Core_Score', 'Combined_Personal_Score',
        
        # Other taste / studio context
        'Franchise_Avg_Score', 'Rated_Avg_Score', 'Company_Avg_Score',
        ...
        # Sub-genre pattern flags (binary)
...
def _empirical_star_rate(scores, y_star, lo, hi, fallback):
    mask = (scores >= lo) & (scores < hi)
    if mask.sum() == 0:
        return fallback
    return float(np.asarray(y_star, dtype=int)[mask].mean())


def fit_star_probability_bundle(score_model, star_model, X_scaled, y_scores, y_star, training_scores):
    """P(star): linear in Predicted Score (anchors from your ratings) + classifier nudge."""
    kfold = KFold(n_splits=5, shuffle=True, random_state=42)
    oof_scores_raw = cross_val_predict(score_model, X_scaled, y_scores, cv=kfold)
    oof_scores = calibrate_predictions(oof_scores_raw, training_scores)
    oof_clf = cross_val_predict(
        star_model, X_scaled, y_star, cv=kfold, method='predict_proba',
    )[:, 1]

    clf_iso = IsotonicRegression(out_of_bounds='clip')
    clf_iso.fit(oof_clf, y_star)

    base_rate = float(y_star.mean())
    p_at_lo = _empirical_star_rate(
        oof_scores, y_star, STAR_SCORE_ANCHOR_LO - 0.35, STAR_SCORE_ANCHOR_LO + 0.35, base_rate,
    )
    p_at_hi = _empirical_star_rate(
        oof_scores, y_star, STAR_SCORE_ANCHOR_HI - 0.5, 10.5, max(p_at_lo + 0.2, base_rate + 0.25),
    )
    p_at_lo = float(np.clip(p_at_lo, STAR_PROB_FLOOR, 0.55))
    p_at_hi = float(np.clip(p_at_hi, p_at_lo + 0.12, STAR_PROB_CAP - 0.02))

    bundle = {
        'star_p_at_lo': p_at_lo,
        'star_p_at_hi': p_at_hi,
        'star_score_anchor_lo': STAR_SCORE_ANCHOR_LO,
        'star_score_anchor_hi': STAR_SCORE_ANCHOR_HI,
        'star_clf_isotonic': clf_iso,
        'star_base_rate': base_rate,
        'star_prob_cap': STAR_PROB_CAP,
        'star_prob_floor': STAR_PROB_FLOOR,
        'star_clf_nudge': STAR_CLF_NUDGE,
    }
    oof_combined = predict_star_proba(oof_scores, oof_clf, bundle)
    bundle['star_yes_threshold'] = float(np.quantile(oof_combined, 1.0 - base_rate))
    return bundle


def predict_star_proba(predicted_scores, clf_raw_proba, bundle):
    scores = np.asarray(predicted_scores, dtype=float)
    span = bundle['star_score_anchor_hi'] - bundle['star_score_anchor_lo']
    t = (scores - bundle['star_score_anchor_lo']) / span if span > 0 else np.zeros_like(scores)
    p_score = bundle['star_p_at_lo'] + t * (bundle['star_p_at_hi'] - bundle['star_p_at_lo'])
    p_clf = bundle['star_clf_isotonic'].predict(np.asarray(clf_raw_proba, dtype=float))
    nudge = bundle['star_clf_nudge'] * (p_clf - bundle['star_base_rate'])
    p = p_score + nudge
    return np.clip(p, bundle['star_prob_floor'], bundle['star_prob_cap'])


def test_star_models(X, y_star):
    """Cross-validate classifiers for P(My_Score >= STAR_THRESHOLD)."""
    print("\n" + "="*60)
    print(f"TESTING STAR MODELS (My_Score >= {STAR_THRESHOLD})")
    print("="*60)
    n_pos = int(y_star.sum())
    n_neg = len(y_star) - n_pos
    pct = 100.0 * n_pos / len(y_star) if len(y_star) else 0
    print(f"  Training base rate: {n_pos}/{len(y_star)} stars ({pct:.1f}%)")
    ...
    for name, model in models.items():
        try:
            auc = cross_val_score(model, X, y_star, cv=kfold, scoring='roc_auc')
            brier = cross_val_score(model, X, y_star, cv=kfold, scoring='neg_brier_score')
            results.append({
                'name': name, 'model': model,
                'auc': auc.mean(), 'auc_std': auc.std(),
                'brier': -brier.mean(),
            })
            print(f"  {name:25s} AUC={auc.mean():.3f} (+/-{auc.std()*2:.3f})  Brier={-brier.mean():.3f}")
        except Exception as e:
            print(f"  {name:25s} FAILED: {e}")

    results.sort(key=lambda x: x['auc'], reverse=True)
    best = results[0]
    print(f"\n✓ Best star model: {best['name']} (AUC={best['auc']:.3f}, Brier={best['brier']:.3f})")
    return best['model'], best['name']


def train_model(df):
    # Load Director/Actor/Genre scores from Movies Ranks.xlsm (required)
    excel_ranks = load_ranks_from_excel()
    
    # Compute all personal average scores
    personal_scores = compute_personal_scores(df, excel_ranks=excel_ranks)
    ...
    # Print personal score source
    print("\n" + "="*60)
    print("PERSONAL SCORES SOURCE")
    print("="*60)
    print("  Director/Actor/Genre scores: from Movies Ranks.xlsm")
    top_d = sorted(personal_scores['director'].items(), key=lambda x: x[1], reverse=True)[:5]
    print(f"  Top 5 Directors: {', '.join(d for d, _ in top_d)}")
    top_a = sorted(personal_scores['actor'].items(), key=lambda x: x[1], reverse=True)[:5]
    print(f"  Top 5 Actors: {', '.join(a for a, _ in top_a)}")
    top_g = sorted(personal_scores['genre1'].items(), key=lambda x: x[1], reverse=True)[:5]
    print(f"  Top 5 Genres: {', '.join(g for g, _ in top_g)}")
    ...
    y_star = (df['My_Score'] >= STAR_THRESHOLD).astype(int)
    best_star_model, best_star_name = test_star_models(X_scaled, y_star)
    print(f"\nTraining {best_star_name} (star) on full data...")
    best_star_model.fit(X_scaled, y_star)

    training_scores = y.values
    star_bundle = fit_star_probability_bundle(
        best_model, best_star_model, X_scaled, y, y_star, training_scores,
    )
    train_scores_cal = calibrate_predictions(best_model.predict(X_scaled), training_scores)
    train_star = predict_star_proba(
        train_scores_cal, best_star_model.predict_proba(X_scaled)[:, 1], star_bundle,
    )
    train_yes = (
        (train_star >= star_bundle['star_yes_threshold'])
        & (train_scores_cal >= STAR_THRESHOLD)
    ).mean() * 100
    print("\n" + "="*60)
    print("STAR PROBABILITY CALIBRATION")
    print("="*60)
    print(f"  Historical star rate: {star_bundle['star_base_rate']*100:.1f}%")
    print(
        f"  Score ramp: {star_bundle['star_p_at_lo']*100:.0f}% @ {STAR_SCORE_ANCHOR_LO} → "
        f"{star_bundle['star_p_at_hi']*100:.0f}% @ {STAR_SCORE_ANCHOR_HI}  |  "
        f"classifier nudge ±{STAR_CLF_NUDGE:.0%}"
    )
    print(f"  Yes/No: prob >= {star_bundle['star_yes_threshold']:.3f} and Predicted Score >= {STAR_THRESHOLD}")
    print(f"  Training Star Yes: {train_yes:.1f}%  |  max Star %: {train_star.max()*100:.1f}")
    ...
    if hasattr(best_model, 'feature_importances_'):
        ...
        print("\nTop 25 Features (score model):")
        for _, row in importance.head(25).iterrows():
            print(f"  {row['feature']:30s} {row['importance']:.4f}")

    if hasattr(best_star_model, 'feature_importances_'):
        star_imp = pd.DataFrame({
            'feature': feature_cols,
            'importance': best_star_model.feature_importances_
        }).sort_values('importance', ascending=False)
        print("\nTop 15 Features (star model):")
        for _, row in star_imp.head(15).iterrows():
            print(f"  {row['feature']:30s} {row['importance']:.4f}")
    
    model_package = {
        'model': best_model,
        'star_model': best_star_model,
        'scaler': scaler,
        ...
        'star_model_name': best_star_name,
...
def calibrate_predictions(predictions, training_scores):
    """
    Percentile (quantile) calibration: map prediction ranks to your rating distribution.
    So the same fraction of predictions that are above 8 (or 9, etc.) as in your
    actual ratings. Preserves relative ordering of predictions.
    """
    training_scores = np.asarray(training_scores)
    predictions = np.asarray(predictions)
    n_train = len(training_scores)
    n_pred = len(predictions)

    # Percentile calibration: map prediction ranks to your rating distribution
    sorted_training = np.sort(training_scores)
    ranks = np.argsort(np.argsort(predictions)).astype(np.float64)
    if n_pred > 1:
        percentile_rank = ranks / (n_pred - 1)
    else:
        percentile_rank = np.array([0.5])
    # Map to inner range of training scores so extremes aren't too extreme
    trim = max(0.0, min(0.49, CALIBRATION_TRIM))
    low_pct = trim
    high_pct = 1.0 - trim
    # Training scores at low_pct and high_pct percentiles
    low_idx = int(low_pct * (n_train - 1)) if n_train > 1 else 0
    high_idx = int(high_pct * (n_train - 1)) if n_train > 1 else (n_train - 1)
    trimmed_scores = sorted_training[low_idx : high_idx + 1]
    n_trim = len(trimmed_scores)
    # Map prediction percentile [0,1] onto [low_pct, high_pct] of training
    calibrated = np.interp(
        percentile_rank,
        np.linspace(0, 1, n_trim),
        trimmed_scores
    )

    # Blend with raw so user can soften if desired
    strength = max(0.0, min(1.0, CALIBRATION_STRENGTH))
    blended = (1.0 - strength) * predictions + strength * calibrated

    return blended


def predict_watchlist(model_package, watchlist_df, training_scores):
    ...
    # Get raw predictions
    raw_predictions = model_package['model'].predict(X_scaled)
    
    calibrated = calibrate_predictions(raw_predictions, training_scores)
    df['Predicted Score'] = np.round(calibrated, 4).clip(1, 10)
    df['Raw_Predicted'] = np.round(raw_predictions, 4).clip(1, 10)

    raw_star = model_package['star_model'].predict_proba(X_scaled)[:, 1]
    star_proba = predict_star_proba(df['Predicted Score'].values, raw_star, model_package)
    yes_threshold = model_package['star_yes_threshold']
    df['Star Percentage'] = np.round(star_proba * 100, 1)
    df['Star'] = np.where(
        (star_proba >= yes_threshold) & (df['Predicted Score'] >= STAR_THRESHOLD),
        'Yes', 'No',
    )
    
    print(f"\nPredicted {len(df)} movies")
    n_star = (df['Star'] == 'Yes').sum()
    pct_star = 100.0 * n_star / len(df) if len(df) else 0
    print(
        f"  Star Yes: {n_star} ({pct_star:.1f}%)  |  "
        f"mean Star %: {df['Star Percentage'].mean():.1f}  |  "
        f"max Star %: {df['Star Percentage'].max():.1f}"
    )
...
def save_output(df, filepath):
    output_cols = ['Movie', 'Year', 'Runtime', 'Director', 'Actor', 'Actor 2', 'Actor 3',
                   'Genre1', 'Genre2', 'Genre3', 'IMDb', 'RT', 'Metascore',
                   'Rated', 'Oscar_Wins', 'Total_Wins', 'Keywords', 
                   'Predicted Score', 'Star Percentage', 'Star',
                   'Raw_Predicted', 'Created']
    output_cols = [c for c in output_cols if c in df.columns]
    
    df_out = df[output_cols].sort_values('Predicted Score', ascending=False)
    df_out.to_csv(filepath, index=False, na_rep='')
    
    print(f"\n✓ Saved: {filepath}")
    
    print("\n" + "="*60)
    print("TOP 25 PREDICTIONS (calibrated)")
    print("="*60)
    display_cols = ['Movie', 'Predicted Score', 'Star Percentage', 'Star', 'IMDb']
    display_cols = [c for c in display_cols if c in df_out.columns]
    print(df_out[display_cols].head(25).to_string(index=False))

    if 'Star Percentage' in df_out.columns:
        print("\n" + "="*60)
        print("TOP 25 BY STAR PERCENTAGE")
        print("="*60)
        star_cols = [c for c in ['Movie', 'Star Percentage', 'Star', 'Predicted Score'] if c in df_out.columns]
        print(df_out.sort_values('Star Percentage', ascending=False)[star_cols].head(25).to_string(index=False))
    
    print("\n" + "="*60)
    print("BOTTOM 15 PREDICTIONS")
    print("="*60)
    print(df_out[display_cols].tail(15).to_string(index=False))
...
def main():
    ...
    print("="*60)
    print("MOVIE SCORE PREDICTOR V6")
    print("="*60)
    ...
    # Pass training scores for calibration
    training_scores = ratings_df['My_Score'].values
    predictions_df = predict_watchlist(model_package, watchlist_df, training_scores)
